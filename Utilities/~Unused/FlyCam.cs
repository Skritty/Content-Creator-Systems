...
public class FlyCam : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (!fileBrowser.activeInHierarchy)
        {
            Debug.Log(result.Count);
            if (Input.GetMouseButtonDown(1) && result.Count == 0)
            {
            ...
            if (!rotateOnlyIfMousedown ||
                (rotateOnlyIfMousedown && flying && Input.GetMouseButton(1)))
            {
                lastMouse = Input.mousePosition - lastMouse;
                lastMouse = new Vector3(-lastMouse.y * camSens, lastMouse.x * camSens, 0);
                lastMouse = new Vector3(transform.eulerAngles.x + lastMouse.x, transform.eulerAngles.y + lastMouse.y, 0);
                transform.eulerAngles += (lastMouse - transform.eulerAngles) * 0.1f;
            ...
            if (Input.GetKey(KeyCode.Space)
                || (movementStaysFlat && !(rotateOnlyIfMousedown && flying && Input.GetMouseButton(1))))
            { //If player wants to move on X and Z axis only
                transform.Translate(p / 30);

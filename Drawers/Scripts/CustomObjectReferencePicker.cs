...
public class CustomObjectReferencePicker : SkinnedWindow, RuntimeInspectorNamespace.IListViewAdapter
{
	private const string SPRITE_ATLAS_PREFIX = "SpriteAtlasTexture-";

	private static CustomObjectReferencePicker m_instance;
	public static CustomObjectReferencePicker Instance
	{
		get
		{
			if (!m_instance)
			{
				m_instance = Instantiate(Resources.Load<CustomObjectReferencePicker>("CustomObjectReferencePicker"));
...
#pragma warning disable 0649
	...
	[SerializeField]
	private RuntimeInspectorNamespace.RecycledListView listView;
...
#pragma warning restore 0649
	...
	int RuntimeInspectorNamespace.IListViewAdapter.Count { get { return filteredReferences.Count; } }
	float RuntimeInspectorNamespace.IListViewAdapter.ItemHeight { get { return Skin.LineHeight; } }
	...
	public void Import()
	{
		//FileBrowser.SetFilters(true, new string[] { ".png", ".jpg", ".jpeg" });
		//FileBrowser.ShowLoadDialog((path) => { TextureImport(path); }, null,
		//FileBrowser.PickMode.Files, true, loadButtonText: "Load");

		var extensions = new[] {
			new ExtensionFilter("Image Files", "png", "jpg", "jpeg" ),
			new ExtensionFilter("All Files", "*" ),
		};
		IList<ItemWithStream> streams = StandaloneFileBrowser.OpenFilePanel("Import", "", extensions, false);
		if (streams.Count > 0)
		{
			TextureImport(streams);
		}
	}

	public void TextureImport(IList<ItemWithStream> streams)
	{
		foreach (ItemWithStream stream in streams)
		{
			//file setup
			string fileName = Path.GetFileName(stream.Name);
			string sourceFile = stream.Name;
			string targetPath = ProjectManager.Instance.projectDirectory.FullName + "/Textures";
			...
			if (ProjectManager.Instance.references.textures.ContainsKey(matData.matName)) continue;
	...
	public void CreateMaterial(MaterialData matData)
	{
		...
		ProjectManager.Instance.references.textures.Add(matData.matName, newMat.mainTexture);

		this.references.Add(newMat.mainTexture);
	...
	protected override void RefreshSkin()
	{
		...
		importButton.SetSkinButton(Skin);
	...
	RecycledListItem RuntimeInspectorNamespace.IListViewAdapter.CreateItem(Transform parent)
	{
	...
	void RuntimeInspectorNamespace.IListViewAdapter.SetItemContent(RecycledListItem item)
	{
	...
	void RuntimeInspectorNamespace.IListViewAdapter.OnItemClicked(RecycledListItem item)
	{
